...
def copy_shared_library(tf_repo_path):
    for cmd in [
            ['sudo', 'cp', 'bazel-bin/tensorflow/libtensorflow_cc.so', LIBRARY_PATH], 
            ['sudo', 'cp', 'bazel-bin/tensorflow/libtensorflow_framework.so', LIBRARY_PATH], 
        ]:
        print(cmd)
        subprocess.check_call(cmd, cwd=tf_repo_path)
    print(Fore.GREEN + 'Successfully copied tensorflow shared library' + Style.RESET_ALL)
...
def copy_headers(tf_repo_path):
    ...
    subprocess.check_call('sudo find {} -type f ! -name "*.h" -delete'.format(
            os.path.join(tf_include_path, 'tensorflow')), shell=True)
    ...
    print(Fore.GREEN +'Successfully copied tensorflow headers!' + Style.RESET_ALL)


def copy_third_party(tf_repo_path):
    tf_include_path = os.path.join(INCLUDE_PATH, 'google', 'tensorflow')
    for cmd in [
        ['sudo', 'cp', '--recursive', 'third_party', tf_include_path],
        ['sudo', 'rm', '--recursive', os.path.join(tf_include_path, 'third_party/py')]
          ]:
        print(cmd)
        subprocess.check_call(cmd, cwd=tf_repo_path)
    print(Fore.GREEN +'Successfully copied third_party headers!' + Style.RESET_ALL)
